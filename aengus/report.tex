...
to 2.8M at depth = 1---a \textbf{97.7\% reduction}. Increasing depth beyond 1
yields no further improvement: the effective miss count is essentially constant
at 2.8M for $D \in \{1,2,4,8\}$. The remaining 2.8M misses are compulsory
cold misses on the first pass through each array that no amount of prefetching
can eliminate.

This saturation is a direct consequence of the simulator being
\emph{functional} rather than timing-based. The hit condition in
\texttt{stream\_buf::access} checks only whether the demand address matches
\texttt{streams[i].head}; there is no model of prefetch latency or memory
access time. A prefetched line is treated as immediately available the moment
it is requested, regardless of depth. As a result, depth has \emph{no effect}
on hit rate in this simulator---it only controls how many prefetch requests are
issued to L2 per allocation and per hit. In real hardware, depth would matter
significantly: a deeper buffer keeps more lines simultaneously in flight from
L2, providing a longer runway to hide L2 latency between successive sequential
accesses. A depth-1 buffer in real hardware would stall whenever the
inter-access time is shorter than the L2 round-trip latency.

L2 bandwidth grows modestly with depth for \texttt{libquantum} (121M to 144M
at $D=8$, an 18.6\% increase) because nearly every prefetched line is
eventually consumed---the prefetch traffic replaces what would otherwise have
been demand misses rather than adding net new traffic.
...
the no-buffer baseline, with essentially no benefit. 
